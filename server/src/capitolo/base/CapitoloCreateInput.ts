/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DescrizioneCreateNestedManyWithoutCapitolosInput } from "./DescrizioneCreateNestedManyWithoutCapitolosInput";
import { ValidateNested, IsOptional, IsInt, IsString } from "class-validator";
import { Type } from "class-transformer";
@InputType()
class CapitoloCreateInput {
  @ApiProperty({
    required: false,
    type: () => DescrizioneCreateNestedManyWithoutCapitolosInput,
  })
  @ValidateNested()
  @Type(() => DescrizioneCreateNestedManyWithoutCapitolosInput)
  @IsOptional()
  @Field(() => DescrizioneCreateNestedManyWithoutCapitolosInput, {
    nullable: true,
  })
  descriziones?: DescrizioneCreateNestedManyWithoutCapitolosInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  numero?: number | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  testo?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  titolo?: string | null;
}
export { CapitoloCreateInput };
